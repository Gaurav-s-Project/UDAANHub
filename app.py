...
# --- Connect to Sheet on App Startup ---
sheet = backend.connect_to_sheet()
if sheet:
    backend.verify_headers(sheet)
else:
    print("CRITICAL: Failed to connect to Google Sheet on startup.")
...
def update_note():
    ...
    sheet.update_cell(row_number, 18, notes) # Adjusted for new column count
...
def search_student_get():
    ...
    row_number = backend.find_student_row(sheet, search_term)
    # --- FIX: Correct indentation for the if block ---
    if not row_number:
        return f"Student '{search_term}' not found. <a href='/'>Go back</a>."
